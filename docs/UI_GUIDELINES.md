...
- The compact Home readiness card is one full-card link to the existing Training readiness disclosure. It must activate Progress, expand and scroll to that single target after rendering, support tap and keyboard activation, and respect reduced-motion preferences. Opening Progress manually must not force this deep-link position.
...
- A compliance-approved historical pilot must remain visibly exceptional: offer only one named session, require a separate local approval, keep all other historical sessions disabled, and label duplicate verification as reuse of the existing activity rather than another post.
