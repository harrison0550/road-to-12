...
The current Foundation audit maps 62 animation-based active guided names to 55 distinct Road to 12% GIFs and maps Seated Concentration Curl to one supplied two-image movement-sequence guide. Three explicitly approved, review-gated lower-ab Phase 2 movements are also registered. Legacy and alias mappings bring the registry to 61 distinct GIFs across 77 exact-name mappings. The visible Library-only Stationary Bike Setup uses a reviewed static equipment guide. Scope and exact names are recorded in `EXERCISE_MEDIA_AUDIT.md`. The registry must not pre-create or map speculative media for later training phases whose workout definitions are not approved.
