...
Focused exercise surfaces are animation-first when a reviewed GIF exists. The reviewed GIF always starts when the workout or enlarged exercise view opens and can be paused back to its static WebP storyboard. A specifically supplied static movement-sequence guide may instead use `mediaType: "movement-sequence"`, with one primary guide image and a second ordered movement image; it does not pretend to be animated. Exercise-library grids continue using still previews so dozens of GIFs do not animate simultaneously. Larger media views reuse the same state model inside a labelled, focus-managed dialog.
...
Chest presses additionally record optional target-muscle engagement as `{target, rating}`. Strong or Moderate engagement, clean form, completed targets, no discomfort, and the intended RIR range are required before progression. Low or None holds the load; machine-press feedback that the front delts or triceps dominate also holds load and prompts a setup adjustment. An optional activation set may be marked complete in active-session state for resumability, but it is deliberately absent from working-set snapshots, volume, PRs, and progression evidence. `programRevision` on newly started sessions prevents a deployed definition change from rewriting an already-active A/B workout, injecting newly added attachment work into it, adding the Smith Machine Hip Thrust to an older active Full Body C session, or moving a press between days inside an active older revision. Corrective safety/form guidance may replace an inaccurate guide immediately while stable exercise identity preserves history.
...
New strength snapshots use stable IDs from `exercise-identity.js`; display-name changes and aliases must not change historical identity. Prescriptions and actual performance are separate structures. Actual sets remain discrete and retain repetitions, weight, unit, completion state, and observed timestamps. Plate-loaded independent-arm entries may use `weightEntry.mode: "perSide"`; each set preserves `weightPerSide` and derives `totalExternalLoadLb` as exactly twice that value. No machine-arm or carriage weight may be assumed without a measured value. The legacy `weight` field remains the entered value for compatibility. Unknown active time, rest time, or set duration is stored as `null`, never inferred as fact. Legacy snapshots remain readable and are not rewritten.

Single-dumbbell unilateral movements retain `weightEntry.mode: "total"` with `paired: false`; the legacy `weight` value means the one implement used for each arm and is never doubled. Single-stack unilateral cable movements use `weightEntry.mode: "single"`; the saved weight is the selector value on the one active stack and is never doubled across limbs. A replacement prompted by discomfort receives a new stable identity and independent progression history so weights, feedback, or discomfort from the replaced exercise cannot seed the new movement. Program revisions retain the prior definition for older active sessions and completed history.
...
Foundation readiness is a pure projection owned by `adaptive-coaching.js`. A qualified phase session must occur on or after `trainingPhase.startedAt`, be a completed strength session named exactly Full Body A, B, or C, and contain at least one completed working set. Earlier workouts remain in history and progression evidence but never count toward the active phase.

The readiness score is uncapped and weighted as 35% balanced A/B/C exposure, 25% adherence, 20% recent recovery ratings, and 20% qualified strength-session volume. Build review eligibility is stricter than the display score: A/B/C each require four qualified exposures, total qualified strength requires twelve, the latest six qualified strength sessions must all be rated with no more than one Tough/Too Hard/Exhausting rating, adherence must be at least 85%, readiness data quality at least 80%, and at least eight reliable exercise baselines must exist. Cardio records, measurements, muscle engagement, and `healthClearance` are contextual and do not gate the phase transition.

Eligibility only unlocks a review. It never changes the schedule. `acceptBuildPhase` requires an eligible projection, explicit user acceptance, and independently validated Build workout templates. `build-upper-lower-program.js` is the only production Build definition and owns the versioned Upper/Lower templates plus the pure future-schedule activation projection. The superseded three-day prototype is not loaded, cached, or available as a runtime fallback. Acceptance atomically replaces `trainingPhase`, appends an evidence-bearing `phaseTransitions` record, and projects the existing unresolved future schedule from a deterministic Monday anchor. Completed, missed, past, rescheduled, active, and pre-anchor Foundation rows are preserved, and existing schedule IDs are retained so no duplicate workout is inserted. Unified schema 21 and `backup-restore.js` preserve the accepted Build version, transition, template IDs, future schedule, session snapshots, and Extra Activity history. Its reconciliation migration inspects field presence so either intermediate schema-19 shape and schema-20 Build state remain compatible without manufacturing phase transitions, Build activation, or activity records.

Build uses four strength days: Monday Upper A, Tuesday Lower A, Thursday Upper B, and Friday Lower B, with existing Cardio/Recovery on Wednesday, Cardio on Saturday, and Rest/Recovery on Sunday. The strength templates contain 22, 19, 21, and 20 working sets and preserve the approved weekly direct volumes: chest 12, back 12, shoulders 6, biceps 9, triceps 4, quads 9, glutes 8, hamstrings 6, calves 8, and core 8. Build contains no dumbbell Romanian deadlift or heavy unsupported hinge. Existing stable exercise identities, reviewed offline media, equipment-specific load semantics, and approved prescription capture are reused; no speculative exercise identity is created.

The midweek rule is sequence-first: Sunday or an unresolved Monday can anchor the next day/current Monday; once Monday is active or resolved, or acceptance occurs Tuesday through Saturday, activation starts the following unresolved Monday. This avoids surprise midweek additions and partial first weeks. The transition record stores `acceptedAt`, `eligibleAt`, readiness/evidence, program version, and `firstBuildDate`; future generation uses `firstBuildDate`, not merely the acceptance timestamp. Strava OAuth/privacy behavior remains outside scheduling. Manual strength eligibility recognizes Foundation Full Body A/B/C and the canonical Build template IDs for Upper A, Lower A, Upper B, and Lower B; cardio, recovery, incomplete sessions, and Extra Activity remain excluded.

Extra Activity is an additive, unscheduled record boundary owned by `extra-activity.js`. Manual logging remains offline. Optional iFIT screenshot processing requires a disclosure and explicit consent, resizes the selected image in memory, and sends only that image to the authenticated Worker route. The vision model returns constrained labeled plain text; the Worker parses and normalizes it, cross-checks clock/duration, pace/distance, and calorie consistency, and returns confidence and warnings for editable review. Incomplete extraction falls back to manual completion rather than failing the workflow. Saving always requires explicit confirmation. Screenshot bytes and raw model output are excluded from local state, D1, and backups. Extra Activity does not complete or move scheduled training, does not enter readiness or strength progression, and is never eligible for Road to 12% Strava posting.
...
Strava Phase 1 remains a pure local projection. `strava-strength-payload.js` converts an eligible completed Full Body A/B/C snapshot to preview metadata and Strava JSON 1.0 sets. It owns equipment-aware external-load normalization, including doubling an explicitly recorded per-side GMWD plate load without adding machine weight, and never reads storage, renders UI, or performs network work. `strava-sync-state.js` owns canonical provider statuses, allowed transitions, authoritative backend reconciliation, and protective provider-state merging. `exercise-identity.js` owns the bounded documented-token allowlist. `app.js` may display the projection but must not reconstruct or duplicate these rules.
...
Deleted historical sessions remain ineligible for reposting by default. The explicitly approved first-pilot exception is local and bounded: `strava-data-boundary.js` selects only the newest eligible tombstoned Full Body session, the user re-approves that one session, and a consumed-session marker prevents any later historical candidate. A fresh deterministic external ID is derived from the unchanged Road to 12% session ID. Confirmed provider state created after renewed consent may survive reload; a later disconnect removes it again. No other historical session, cardio session, or background process is enabled.
