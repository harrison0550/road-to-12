...
if (!/const ROAD12_SCHEMA_VERSION=14;/.test(app)) {
...
if (!/version:8,[\s\S]*?trainingPhase[\s\S]*?measurementHistory[\s\S]*?cardioHistory[\s\S]*?value\.schemaVersion=8;/.test(app)||!/version:9,[\s\S]*?exerciseFeedback[\s\S]*?approvedProgressions[\s\S]*?value\.schemaVersion=9;/.test(app)||!/version:10,[\s\S]*?cardioTimers[\s\S]*?value\.schemaVersion=10;/.test(app)||!/version:11,[\s\S]*?exerciseTimings[\s\S]*?value\.schemaVersion=11;/.test(app)||!/version:12,[\s\S]*?adherenceBaselineDate[\s\S]*?value\.schemaVersion=12;/.test(app)||!/version:13,[\s\S]*?dumbbellPairWeights:\[10,15,20,25\][\s\S]*?value\.schemaVersion=13;/.test(app)||!/version:14,[\s\S]*?lowerAbsProgram[\s\S]*?value\.schemaVersion=14;/.test(app)) {
