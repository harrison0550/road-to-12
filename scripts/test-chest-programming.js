...
assert.match(app, /function fullBodyCWorkout\(includeHipThrust=true\)/, "Full Body C must remain a separate workout definition with a revision-gated hip-thrust addition");
...
assert.match(app, /includeCurrentAttachments=!activeSession\|\|\[PREVIOUS_FOUNDATION_PROGRAM_REVISION,FOUNDATION_PROGRAM_REVISION\]\.includes\(state\.currentSession\.programRevision\)/, "new additive attachment work must remain present for compatible active workout revisions");
