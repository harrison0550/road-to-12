...
const state={schemaVersion:20,preferredName:"Andy",weight:210,waist:40,sessions:1,tab:"progress",step:2,history:[{id:"session-1",name:"Full Body A",completedAt:"2026-08-15T12:00:00.000Z",exercises:[{exerciseId:"smith-machine-squat",prescription:{sets:3,reps:10},sets:[{setNumber:1,repetitions:10,weight:70,completed:true}],externalMappings:{strava:{sportType:"WeightTraining"}}}],externalSync:{strava:{status:"NOT_SYNCED"}}}],workoutRatings:{"session-1":"Good"},dailyCheckins:{},achievements:{},trainingProfile:{age:40},trainingPhase:{id:"build",programVersion:"build-v1"},phaseTransitions:[{id:"foundation-build-test",from:"foundation",to:"build",eligibleAt:"2026-09-01",acceptedAt:"2026-09-02T12:00:00.000Z",readinessScore:100,evidenceSnapshot:{score:100,gates:{}}}],buildProgramVersion:"build-v1",measurementHistory:[{id:"m-1",date:"2026-08-15",weight:210,waist:40}],bodyMeasurements:[{id:"bm-1",source:"manual",timestamp:"2026-08-15T12:00:00.000Z",weight:210,waist:40},{id:"wyze-1",source:"wyze-import",timestamp:"2026-08-16T12:00:00.000Z",sourceTimestamp:"2026.08.16 8:00 AM",sourceRecordNumber:"9",weight:209.8,weightLb:209.8,weightKg:95.2,bmi:27.1,bodyFatPercent:25,muscleMassLb:145,muscleMassPercent:69.1,leanBodyMassLb:157.3,boneMassLb:8.9,bmrKcal:1880,fatMassLb:52.5}],cardioHistory:[{id:"c-1",sessionId:"session-1",name:"Warm-up",actualDurationMinutes:8}],approvedProgressions:{"smith-machine-squat":{exerciseId:"smith-machine-squat",status:"approved",prescription:{sets:3,reps:10,weight:75}}},lowerAbsProgram:{version:1,phase:1,status:"active",completedSessionIds:[],phase2ReadyAt:null,phase2AcceptedAt:null,completedAt:null},equipment:{dumbbells:true,dumbbellPairWeights:[10,15,20,25]},attachmentPhotos:{},workoutSessions:[{id:"scheduled-1",plannedDate:"2026-08-15",scheduledDate:"2026-08-16",status:"rescheduled",phaseId:"build",templateId:"build-full-body-a",templateVersion:"build-v1"}],scheduleActivatedDate:"2026-08-01",adherenceBaselineDate:"2026-08-20",currentSession:{id:"active-1",sessionPrescriptions:{},templateId:"build-full-body-a",templateVersion:"build-v1"},logs:{},exerciseFeedback:{},cardioTimers:{},exerciseTimings:{}};
const payload=backup.create({version:"13.2.0",build:"2026.08.29.1"},state,20);
...
const validated=backup.validate(JSON.parse(JSON.stringify(payload)),20);
...
assert.throws(()=>backup.validate({...payload,state:{...payload.state,history:"bad"}},20),/history/i);
assert.throws(()=>backup.validate({...payload,schemaVersion:21,state:{...payload.state,schemaVersion:21}},20),/newer/i);
assert.throws(()=>backup.validate({...payload,state:{...payload.state,bodyMeasurements:[{source:"wyze-bluetooth",timestamp:"2026-08-15T12:00:00.000Z",weight:210}]}},20),/body measurement/i);
assert.throws(()=>backup.validate({...payload,format:"other-app-backup"},20),/unknown backup format/i);
assert.throws(()=>backup.validate({...payload,build:null},20),/release metadata/i);
const untouched=JSON.stringify(state);
try{backup.validate({state:{history:null}},19);}catch(_error){}
