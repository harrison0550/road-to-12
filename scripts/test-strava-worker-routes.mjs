...
class FakeD1{
  constructor(){this.installations=new Map();this.nonces=new Map();this.states=new Map();this.connections=new Map();this.uploads=new Map();}
  prepare(sql){
    ...
    return {bind(...a){return {
      ...
      async run(){
        ...
        if(q.startsWith("UPDATE oauth_states")){const row=db.states.get(a[1]);if(!row||row.used_at)return {meta:{changes:0}};row.used_at=a[0];return {meta:{changes:1}};}
        ...
        if(q.startsWith("UPDATE strava_connections SET access_token_cipher = NULL")){const row=db.connections.get(a[1]);Object.assign(row,{access_token_cipher:null,refresh_token_cipher:null,access_token_expires_at:null,disconnected_at:a[0],requires_reauth:0});return {meta:{changes:1}};}
...
const env={DB:db,PWA_ORIGIN:pwaOrigin,OAUTH_REDIRECT_URI:`${workerOrigin}/api/strava/callback`,STRAVA_CLIENT_ID:"client-id",STRAVA_CLIENT_SECRET:"client-secret",TOKEN_ENCRYPTION_KEY:encryptionKey};
...
try{
  ...
  response=await worker.fetch(new Request(`${workerOrigin}/api/strava/callback?state=${encodeURIComponent(oauthState)}&code=code`),env);
  assert.equal(response.status,400,"OAuth state must be one-time-use");
  response=await worker.fetch(await signedRequest("/api/strava/status"),env);
  ...
  response=await worker.fetch(await signedRequest("/api/strava/disconnect",{method:"POST",body:{}}),env);assert.equal(response.status,200);assert.equal(revokeCalls,1);
